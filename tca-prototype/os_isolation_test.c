 ...
 * Saves standard Callee-Saved registers (ra, s0-s11) PLUS the TCA hardware
 * state.
 */
__attribute__((naked)) void switch_context(ThreadContext *prev,
                                           ThreadContext *next) {
  __asm__ volatile(
      /* 1. Allocate stack space for 14 standard registers + 2 TCA CSRs = 16 * 8
         = 128 bytes */
      "addi sp, sp, -128\n"
...
void _start() {
  ...
  // 1. Thread A (Malicious) reads 0x87E00000. Hardware 0x801 (Taint) flips
  // to 1.
  ...
  *tx_trigger = 1;
  ...
  uart_puts("[THREAD A] Run complete (Should not be reached if NIC drops the "
            "transmission).\n");
