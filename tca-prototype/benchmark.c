...
static void uart_putc(char c) { *(volatile char *)UART_BASE = c; }
...
__attribute__((noinline)) void simulate_ebpf_lsm_hook(void) {
  ...
  // context restore
  volatile uint64_t r0 = ctx_save[0];
  (void)r0;
  volatile uint64_t r1 = ctx_save[1];
  (void)r1;
  volatile uint64_t r2 = ctx_save[2];
  (void)r2;
  volatile uint64_t r3 = ctx_save[3];
  (void)r3;
}
