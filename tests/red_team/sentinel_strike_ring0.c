...
void *lsm_egress_violator(void *arg) {
  ...
  printf(
      "[+] LSM Thread: Successfully intercepted %d/%d connections (-EPERM).\n",
      blocked_count, flood_count);
  return NULL;
}

// --- THREAD 2: XDP Ingress Flooder ---
// Generates a massive burst of raw UDP packets to stress the XDP drop logic and
// ring buffer. Expectation: XDP silently drops packets at wire-speed before
// they hit the Linux network stack.
void *xdp_ingress_flooder(void *arg) {
  ...
  for (int i = 0; i < flood_count; i++) {
    sendto(sock, payload, strlen(payload), 0, (struct sockaddr *)&target,
           sizeof(target));
  }

  close(sock);
  printf("[+] XDP Thread: Flood complete. Awaiting Prometheus telemetry "
         "verification.\n");
  return NULL;
}
...
void self_taint() {
  ...
  if (connect(sock, (struct sockaddr *)&addr, sizeof(addr)) == 0) {
    char msg[256];
    sprintf(msg,
            "{\"command\": \"UPDATE_TAINT\", \"data\": {\"pid\": %d, "
            "\"taint_level\": 4}}\n",
            getpid());
