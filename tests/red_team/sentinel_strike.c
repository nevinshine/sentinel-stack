...
void *attack_lsm(void *arg) {
  ...
  do {
    ...
    if (flood_mode)
      usleep(500); // 0.5ms delay to prevent total fd exhaustion
  } while (flood_mode);
...
void *attack_mei(void *arg) {
  ...
  // Intel AMT Remote Control GUID: {0x8E6A6715, 0x9ABC, 0x4043, {0x88, 0xEF,
  // 0x9E, 0x38, 0xC6, 0x29, 0x4D, 0x07}}
  uint8_t amt_guid[16] = {0x15, 0x67, 0x6A, 0x8E, 0xBC, 0x9A, 0x43, 0x40,
  ...
  do {
    ...
    if (fd >= 0) {
      ...
      if (errno == EPERM || errno == EACCES) {
        ...
        if (!flood_mode)
          break;  // Don't spam if it's completely missing
        sleep(1); // Backoff
      }
    }
...
void *attack_xdp(void *arg) {
  ...
  inet_pton(AF_INET, "127.0.0.1",
            &target.sin_addr); // Loopback or dummy interface
  ...
  do {
    ...
    if (sock >= 0) {
      sendto(sock, payload, sizeof(payload), 0, (struct sockaddr *)&target,
             sizeof(target));
...
int main(int argc, char **argv) {
  ...
  if (gethostname(hostname, sizeof(hostname)) == 0) {
    if (strcmp(hostname, "intelhost") != 0 &&
        strcmp(hostname, "raspberrypi") != 0) {
      fprintf(stderr,
              "[!] FATAL: Execution aborted to protect host machine.\n");
      fprintf(stderr,
              "[!] Hostname '%s' does not match target lab environment "
              "('intelhost' or 'raspberrypi').\n",
              hostname);
      exit(1);
    }
  }
  ...
  if (!flood_mode) {
    ...
    printf("XDP TX Packets : %d (Check XDP hook telemetry for drop "
           "verification)\n",
           xdp_packets_sent);
    printf("---------------------\n");
  } else {
