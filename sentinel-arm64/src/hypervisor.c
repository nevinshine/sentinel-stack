...
void handle_sync_lower(uint64_t esr, uint64_t far, uint64_t *regs) {
  ...
  if (ec == ESR_EC_DATA_ABORT_L) {
    ...
    /* Check if the fault is explicitly a Stage-2 Translation or Permission
     * Fault */
    if ((dfsc >= DFSC_S2_FAULT_L0 && dfsc <= DFSC_S2_FAULT_L3) ||
       ...
       */
      if (is_write) {
        uart_puts("[EL2-SECURITY] STAGE-2 FAULT TRAPPED: Malicious write "
                  "attempt blocked!\n");
    ...
    uart_puts("CRITICAL: Malicious Instruction Execution Attempt Intercepted "
              "at EL2.\n");
    ...
    uint64_t pgd_base =
        (guest_va & (1ULL << 55)) ? (ttbr1 & ~0xFFF) : (ttbr0 & ~0xFFF);
...
void hypervisor_main(void) {
  ...
  uart_puts(
      "[Sentinel-VMI] Initialization complete. Dropping to EL1 Guest...\n");
