 ...
 * attempting to interact with restricted Intel Management Engine services via the
...
static const uuid_le AMT_REMOTE_CONTROL_GUID = UUID_LE(0x12F80028, 0xB4B7, 0x4B2D,
						     0xAC, 0xA8, 0x46, 0xE0, 0xFF, 0x65, 0x81, 0x4C);

static const uuid_le AMT_SOL_GUID = UUID_LE(0xFB3B192E, 0xE714, 0x44A4,
					   0x8B, 0x22, 0x55, 0x82, 0x23, 0xDE, 0x6F, 0xF4);

static const uuid_le ICC_CLOCK_CONTROL_GUID = UUID_LE(0xF934D0F2, 0x3E42, 0x402E,
						    0x8C, 0xDF, 0x73, 0x01, 0x02, 0xAA, 0x13, 0xD1);

static const uuid_le MKHI_GUID = UUID_LE(0x8E6A6715, 0x9ABC, 0x4043,
...
#define TARGET_CLIENTS_COUNT (sizeof(target_clients) / sizeof(struct target_client))

static void usage(const char *progname)
{
	fprintf(stderr, "Usage: %s [OPTIONS]\n", progname);
	fprintf(stderr, "Sentinel Live-Fire HECI Interdiction Validation Utility\n\n");
	fprintf(stderr, "Options:\n");
	fprintf(stderr, "  -d, --device <path>   Use custom HECI character device (default: /dev/mei0)\n");
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < (int)TARGET_CLIENTS_COUNT; i++) {
		...
		printf("\n------------------------------------------------------------\n");
		printf("[*] Client: %s (%s)\n", client->name,
		       client->is_restricted ? "RESTRICTED - Should be Blocked" : "NON-RESTRICTED - Should Pass");

		// Format UUID string for output using individual byte elements of uuid_le
		snprintf(uuid_str, sizeof(uuid_str),
			 "%02X%02X%02X%02X-%02X%02X-%02X%02X-%02X%02X-%02X%02X%02X%02X%02X%02X",
			 client->uuid->b[3], client->uuid->b[2], client->uuid->b[1], client->uuid->b[0],
			 client->uuid->b[5], client->uuid->b[4],
			 client->uuid->b[7], client->uuid->b[6],
			 client->uuid->b[8], client->uuid->b[9],
			 client->uuid->b[10], client->uuid->b[11],
			 client->uuid->b[12], client->uuid->b[13],
			 client->uuid->b[14], client->uuid->b[15]);
		...
		if (ret < 0) {
			...
			if (err == EPERM) {
				printf("[SUCCESS] EPERM received! sentinel_mei_hook blocked the connection.\n");
			}
			close(fd);
			continue;
		}
		...
		if (written < 0) {
			...
			if (err == EPERM) {
				printf("[SUCCESS] EPERM received on HECI send! sentinel_mei_hook successfully interdicted write.\n");
			}
		} else {
