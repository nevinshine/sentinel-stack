...
static void usage(const char *progname)
{
	...
	fprintf(stderr, "  --enable-me-disable    Acknowledge the risks and execute the soft-disable\n");
...
int main(int argc, char *argv[])
{
	...
	if (bytes < 0) {
		...
		if (errno == EPERM) {
			fprintf(stderr, "Error: Command blocked (EPERM). Is sentinel_mei_hook loaded with strict_mkhi=1?\n");
		}
		close(fd);
		return EXIT_FAILURE;
	}
	...
	if (resp.hdr.result == 0) {
		...
		printf("[SUCCESS] The ME will enter a dormant state after the next global reset.\n");
	} else {
