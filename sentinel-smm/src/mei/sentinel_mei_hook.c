 ...
 */
static bool
sentinel_mei_extract_guid(struct pt_regs *regs, guid_t *out_guid)
{
	 ...
	 */
	if (copy_from_kernel_nofault(&me_cl_addr,
				     (void *)cldev + offsetof(struct mei_cl_device, me_cl),
	 ...
	 */
	if (copy_from_kernel_nofault(out_guid,
				     me_cl_ptr + MEI_ME_CLIENT_GUID_OFFSET,
				     sizeof(guid_t)))
		return false;
 ...
 */
static int
sentinel_mei_send_pre(struct kprobe *p, struct pt_regs *regs)
{
	guid_t guid;
 ...
 */
static int
sentinel_mei_send_ret_entry(struct kretprobe_instance *ri,
			    struct pt_regs *regs)
{
...
static int
sentinel_mei_send_ret_handler(struct kretprobe_instance *ri,
			      struct pt_regs *regs)
{
	struct sentinel_mei_kretprobe_data *data;
 ...
 */
static int
sentinel_mei_recv_pre(struct kprobe *p, struct pt_regs *regs)
{
	guid_t guid;
	const char *client_name = "Unknown";
	size_t length;
