...
class TelosDNSProxy:
    ...
    def start(self):
        ...
        thread.start()
        log.info(f"[PASS] DNS Proxy listening on {self.host}:{self.port} (Upstream: {self.upstream_dns})")
    ...
    def _handle_request(self, data, addr):
        try:
            ...
            # Ignore IPv6 AAAA records entirely since AF_INET6 is blackholed
            if qtype == 'AAAA':
                reply = request.reply()
                    ...
                    for rr in bad_parsed.rr:
                        if rr.rtype == QTYPE.A:
                            ...
                            try:
                                ...
                                log.error(f"[DNS] [WARN] Failed to reach Hyperion RPC: {e}")
                except Exception as e:
                    log.error(f"[DNS] [WARN] Failed to extract malicious IPs: {e}")
            ...
            for rr in reply.rr:
                if rr.rtype == QTYPE.A:
                    ...
                    if success:
                        ...
                        log.error(f"[DNS] [FAIL] IPC Failed to open firewall for {ip_str}")
