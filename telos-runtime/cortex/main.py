...
class TelosControlService(protocol_pb2_grpc.TelosControlServicer):
    ...
    def DeclareIntent(self, request: protocol_pb2.IntentRequest,
        ...
        if allowed:
            ...
            log.warning(f"[FAIL] Intent DENIED: {reason}")
            # [PHASE 5: Intent-Based Execution]
            if exec_actions:
                self.ipc.send_update_exec(request.agent_pid, [], mode=1)
...
class CortexServer:
    ...
    def _load_policy(self) -> dict:
        ...
        try:
            with open(self.policy_path, 'r') as f:
                ...
                log.info(f"[PASS] Loaded policy from {self.policy_path}")
                return policy
    ...
    def stop(self):
        ...
        if hasattr(self, 'dns_proxy'):
            self.dns_proxy.stop()
            log.info("[PASS] DNS Proxy stopped")
        
        if self.server:
            self.server.stop(grace=5)
            log.info("[PASS] gRPC server stopped")
        
        if self.ipc:
            self.ipc.close()
            log.info("[PASS] IPC connection closed")
...
def sync_filesystem_policy(config: dict, ipc: CoreIPCClient):
    ...
    log.info(f"[PASS] Protected {count} sensitive inodes")
...
def sync_network_policy(config: dict, ipc: CoreIPCClient):
    ...
    # Also resolve blocked hosts? No, the policy is "Deny All except Allowed"
    # So only the allowlist needs to be pushed.
    ...
    for host in allowed_hosts:
        try:
            ...
            # For M2 POC, resolve once at startup.
            ...
            for _, _, _, _, sockaddr in addr_info:
                ...
                # BPF usually expects network byte order if dealing with SKB, but socket structs are used.
                # Let's check bpf_lsm.c: dest_ip = addr->sin_addr.s_addr;
                # sin_addr.s_addr is Network Byte Order (Big Endian).
                # So Network Byte Order must be pushed.
    ...
    log.info(f"[PASS] Allowed {count} network destinations")
