...
def main():
    ...
    for cmd in SAFE_COMMANDS:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"  [WARN]  {cmd_str} (exit code {result.returncode})")
                failed += 1
        except PermissionError:
            print(f"  [FAIL] {cmd_str} - BLOCKED!")
            blocked += 1
    ...
    if blocked > 0:
        print()
        print("[FAIL] CRITICAL: Normal processes were blocked!")
        print("   This is a bug - TELOS should only block tainted processes.")
        return 1
    elif failed > 0:
        print()
        print("[WARN]  Some commands failed (not blocked, just errors)")
        return 0
    else:
        print()
