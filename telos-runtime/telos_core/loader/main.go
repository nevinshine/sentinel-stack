...
type ProcessViolation struct {
	...
	TaintSignature uint64 `json:"taint_signature"`
}
...
type SentinelEvent struct {
	...
	TimestampNs uint64
	PidTgid     uint64
	Data        [16]byte // 16-byte raw C union size in sentinel_ipc.h
}

func (e *SentinelEvent) MarshalJSON() ([]byte, error) {
	type SentinelEventJSON struct {
		EventType   uint32            `json:"event_type"`
		TimestampNs uint64            `json:"timestamp_ns"`
		PidTgid     uint64            `json:"pid_tgid"`
		Violation   *ProcessViolation `json:"process_violation,omitempty"`
		Drop        *NetworkDrop      `json:"network_drop,omitempty"`
	}
	j := SentinelEventJSON{
	...
	if e.EventType == 1 { // SENTINEL_EVENT_PROCESS_VIOLATION
		var pv ProcessViolation
		_ = binary.Read(bytes.NewReader(e.Data[:]), binary.LittleEndian, &pv)
		j.Violation = &pv
	} else if e.EventType == 2 { // SENTINEL_EVENT_NETWORK_DROP
		var nd NetworkDrop
		_ = binary.Read(bytes.NewReader(e.Data[:]), binary.LittleEndian, &nd)
		j.Drop = &nd
	}
...
type BPFMaps struct {
	...
	Events          *ebpf.Map
	SentinelEvents  *ebpf.Map
	VmiAlertMap     *ebpf.Map
	PccAuthMap      *ebpf.Map // [Pillar 5] PCC AOT Map
}
...
type TelosDaemon struct {
	...
	// Pillar 3: Telemetry Pipeline
	siemForwarder *SIEMForwarder
	telemetryChan chan SentinelEvent
	siemIP        string
}
...
var (
	...
	metricTaintViolations = prometheus.NewCounter(
		prometheus.CounterOpts{
			Name: "telos_taint_violations_total",
			Help: "Total number of synchronous Information Flow Control process taint violations.",
		},
	)
	metricPacketsDropped = prometheus.NewCounter(
		prometheus.CounterOpts{
			Name: "hyperion_packets_dropped_total",
			Help: "Total number of network transport packets dropped by the Hyperion XDP engine.",
		},
	)
)
...
func init() {
	...
	prometheus.MustRegister(metricTaintViolations)
...
func NewTelosDaemon(socketPath, bpfObjPath string) *TelosDaemon {
	...
	return &TelosDaemon{
		...
		siemForwarder:    NewSIEMForwarder(),
		telemetryChan:    make(chan SentinelEvent, 1000),
		siemIP:           os.Getenv("TELOS_SIEM_IP"),
	}
}
...
func (d *TelosDaemon) Start() error {
	...
	log.Println("[PASS] Removed memory lock limits")
	...
	log.Println("[PASS] eBPF program loaded and attached")
	...
	log.Println("[PASS] Default config initialized")

	// Start Unix socket server
	if err := d.startSocketServer(); err != nil {
		return fmt.Errorf("failed to start socket server: %w", err)
	}
	log.Printf("[PASS] Listening on %s", d.socketPath)
	...
	log.Printf("[PASS] Events stream running on %s", d.eventsSocketPath)

	go d.readEvents()
	go d.readSentinelEvents()
	go d.pollVmiAlertMap()
	go d.siemForwarder.Start(context.Background(), d.telemetryChan)
	...
	if err := d.initAlertLogger(); err != nil {
		...
		log.Println("[PASS] NDJSON alert logger active → /var/log/telos/alerts.json")
	}
...
func (d *TelosDaemon) loadBPF() error {
	...
	// Check if a pinned vmi_alert_map already exists to reuse its reference persistently
	var mapReplacements map[string]*ebpf.Map
	vmiAlertMapPath := "/sys/fs/bpf/telos/vmi_alert_map"
	var existingVmiMap *ebpf.Map
	if _, err := os.Stat(vmiAlertMapPath); err == nil {
		existingMap, err := ebpf.LoadPinnedMap(vmiAlertMapPath, nil)
		if err == nil {
			log.Printf("[VMI] Found existing pinned vmi_alert_map at %s. Redirecting eBPF program map references...", vmiAlertMapPath)
			mapReplacements = map[string]*ebpf.Map{
				"vmi_alert_map": existingMap,
			}
			existingVmiMap = existingMap
		} else {
			log.Printf("[VMI] Warning: Failed to load pinned vmi_alert_map: %v", err)
		}
	}
	...
	d.maps = &BPFMaps{
		...
		SentinelEvents:  coll.Maps["sentinel_events"],
		VmiAlertMap:     coll.Maps["vmi_alert_map"],
		PccAuthMap:      coll.Maps["pcc_auth_map"],
	}

	if existingVmiMap != nil {
		d.maps.VmiAlertMap = existingVmiMap
	}
	...
	// Pin sentinel_events map
	if d.maps.SentinelEvents != nil {
		// Ensure to unpin first if exists to prevent "file exists" error
		_ = os.Remove(filepath.Join(bpfPinPath, "sentinel_events"))
		if err := d.maps.SentinelEvents.Pin(filepath.Join(bpfPinPath, "sentinel_events")); err != nil {
			log.Printf("Warning: Failed to pin sentinel_events: %v", err)
		} else {
			log.Printf("[PASS] Pinned sentinel_events to %s", filepath.Join(bpfPinPath, "sentinel_events"))
		}
	}

	// Pin vmi_alert_map persistently
	if d.maps.VmiAlertMap != nil {
		vmiAlertMapPath := "/sys/fs/bpf/telos/vmi_alert_map"
		if _, err := os.Stat(vmiAlertMapPath); err != nil {
			if err := d.maps.VmiAlertMap.Pin(vmiAlertMapPath); err != nil {
				log.Printf("Warning: Failed to pin vmi_alert_map: %v", err)
			} else {
				log.Printf("[PASS] Pinned vmi_alert_map to %s", vmiAlertMapPath)
			}
		} else {
			log.Printf("[PASS] Using persistently pinned vmi_alert_map at %s", vmiAlertMapPath)
		}
	}

	// Pin pcc_auth_map persistently for AOT registration
	if d.maps.PccAuthMap != nil {
		pccMapPath := filepath.Join(bpfPinPath, "pcc_auth_map")
		if _, err := os.Stat(pccMapPath); err != nil {
			if err := d.maps.PccAuthMap.Pin(pccMapPath); err != nil {
				log.Printf("Warning: Failed to pin pcc_auth_map: %v", err)
			} else {
				log.Printf("[PASS] Pinned pcc_auth_map to %s", pccMapPath)
			}
		} else {
			log.Printf("[PASS] Using persistently pinned pcc_auth_map at %s", pccMapPath)
		}
	}
...
func (d *TelosDaemon) startSocketServer() error {
	...
	go func() {
		...
		log.Println("[PASS] Prometheus metrics available on :9094/metrics")
...
func (d *TelosDaemon) readEvents() {
	...
	for {
		...
		// [Phase 4] Trigger NetworkSlam if VMI lockdown occurs
		if event.DescStr == "vmi_lockdown" || event.DescStr == "vmi_lockdown_exec" || event.DescStr == "vmi_lockdown_file" {
			go d.EnforceNetworkSlam(event.PID, d.siemIP)
		}
...
func (d *TelosDaemon) cmdIPCPing() IPCResponse {
	...
	if !d.watchdogActive {
		log.Println("[PASS] Core Watchdog Active: Receiving IPC_PINGS from Cortex.")
		d.watchdogActive = true

		// If previously Failed-Open, reset to Enforcing mode
		key := uint32(0)
		var cfg Config
...
func (d *TelosDaemon) orphanCleanupRoutine() {
	...
	for {
		...
		case <-ticker.C:
			...
			if evicted > 0 {
				log.Printf("[PASS] Orphan Cleanup: Purged %d dead PID states from BPF LRU map", evicted)
			}
...
func (d *TelosDaemon) writeAlertNDJSON(event BPFEvent) {
	...
	if stat, err := d.alertFile.Stat(); err == nil {
		if stat.Size() > alertMaxSizeBytes {
			...
			d.alertFile = f
			log.Println("[PASS] Alert log rotated → alerts.json.1")
		}
	}
...
func (d *TelosDaemon) cmdBulkBlockIPs(data map[string]interface{}) IPCResponse {
	...
	log.Printf("[PASS] Threat Intel: Bulk-blocked %d IPs from feed", blocked)
	return IPCResponse{
		Success: true,
...
// readSentinelEvents consumes the sentinel_events ringbuffer
func (d *TelosDaemon) readSentinelEvents() {
	if d.maps.SentinelEvents == nil {
		log.Println("[Warning] sentinel_events BPF map not present in Collection")
		return
	}

	reader, err := ringbuf.NewReader(d.maps.SentinelEvents)
	if err != nil {
		log.Printf("[Error] Failed to initialize sentinel_events ringbuffer reader: %v", err)
		return
	}
	defer reader.Close()

	log.Println("[PASS] sentinel_events ringbuffer listener fully active")
	...
	for {
		...
		if err != nil {
			...
			log.Printf("[Error] reading from sentinel_events ringbuffer: %v", err)
			continue
		}
		...
		if err != nil {
			log.Printf("[Error] Failed to deserialize structured telemetry payload: %v", err)
			continue
		}

		// Update metrics based on multiplexed event type
		switch event.EventType {
		case 1: // SENTINEL_EVENT_PROCESS_VIOLATION
			metricTaintViolations.Inc()
			if event.PidTgid > 0 {
				pid := uint32(event.PidTgid >> 32)
				go d.EnforceNetworkSlam(pid, d.siemIP)
			}
		case 2: // SENTINEL_EVENT_NETWORK_DROP
			metricPacketsDropped.Inc()
		}

		// Non-blocking write to the SIEM forwarder pipeline
		select {
		case d.telemetryChan <- event:
		default:
			log.Println("[Warning] Telemetry queue full! Dropping user-space record to prevent blocking.")
		}
	}
}

// pollVmiAlertMap periodically checks the vmi_alert_map for new VMI_THREAT_MALICIOUS (2) flags
// and triggers NetworkSlam. This catches alerts injected directly by the hypervisor bypassing LSM events.
func (d *TelosDaemon) pollVmiAlertMap() {
	if d.maps.VmiAlertMap == nil {
		return
	}

	pollMs := 500
	if val := os.Getenv("TELOS_POLL_INTERVAL_MS"); val != "" {
	...
	defer ticker.Stop()
	slammedPIDs := make(map[uint32]bool)
	...
	for {
		...
		case <-ticker.C:
			var pid uint32
			var threatLevel uint32

			activeThreats := make(map[uint32]bool)
			iter := d.maps.VmiAlertMap.Iterate()
			for iter.Next(&pid, &threatLevel) {
				if threatLevel >= 2 {
					activeThreats[pid] = true
					if !slammedPIDs[pid] {
						slammedPIDs[pid] = true
						go d.EnforceNetworkSlam(pid, d.siemIP)
					}
				}
			}

			for cachedPID := range slammedPIDs {
				if !activeThreats[cachedPID] {
					delete(slammedPIDs, cachedPID)
				}
			}
		}
	}
...
const (
	StateClosed CircuitBreakerState = iota // Connected to SIEM gRPC
	StateOpen                              // Disconnected, dumping to disk
)
...
func NewSIEMForwarder() *SIEMForwarder {
	_ = os.MkdirAll("/var/log/telos", 0755)
	file, err := os.OpenFile("/var/log/telos/sentinel_telemetry_backup.json", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0600)
	if err != nil {
		// Fallback to /tmp if permissions are restricted
		file, _ = os.OpenFile("/tmp/sentinel_telemetry_backup.json", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0600)
	}
...
func (sf *SIEMForwarder) Start(ctx context.Context, telemetryChan <-chan SentinelEvent) {
	log.Println("[PASS] SIEM telemetry forwarder active")
	for {
		select {
		case <-ctx.Done():
			return
		case event := <-telemetryChan:
			sf.forward(event)
		}
	}
}

func (sf *SIEMForwarder) forward(event SentinelEvent) {
	sf.stateLock.Lock()
	defer sf.stateLock.Unlock()

	// If the circuit is Open, attempt reconnection periodically
	if sf.state == StateOpen {
		if time.Since(sf.lastRetry) > 10*time.Second {
			log.Println("[SIEM] Attempting to reconnect to gRPC sidecar...")
			if sf.tryReestablishConnection() {
				log.Println("[SIEM] Reconnection successful! Resetting circuit-breaker.")
				sf.state = StateClosed
				sf.failureCount = 0
			} else {
	...
	if sf.state == StateOpen {
		sf.writeToDisk(event)
		return
	}

	// Mock gRPC client call
	err := sf.sendToGRPCSidecar(event)
	if err != nil {
		sf.failureCount++
		log.Printf("[SIEM] gRPC delivery failure (%d/3): %v", sf.failureCount, err)
		if sf.failureCount >= 3 {
			log.Println("!!! SIEM Connection Lost. Tripping Circuit-Breaker to State: OPEN !!!")
			sf.state = StateOpen
			sf.lastRetry = time.Now()
			sf.writeToDisk(event)
		}
...
func (sf *SIEMForwarder) sendToGRPCSidecar(event SentinelEvent) error {
	// Simulated SIEM delivery
	// If the environment variable SIEM_FAIL is set to 1, simulate connection drop!
	if os.Getenv("SIEM_FAIL") == "1" {
		return errors.New("simulated gRPC timeout/connection reset")
	}
	return nil
}

func (sf *SIEMForwarder) tryReestablishConnection() bool {
	// Reconnection simulator
	// If SIEM_FAIL is still "1", reconnection fails!
	if os.Getenv("SIEM_FAIL") == "1" {
		return false
	}
	return true
...
func (sf *SIEMForwarder) writeToDisk(event SentinelEvent) {
	data, _ := json.Marshal(event)
	if sf.backupFile != nil {
		_, _ = sf.backupFile.Write(append(data, '\n'))
	}
}
