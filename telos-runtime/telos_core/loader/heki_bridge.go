...
// HekiDrawbridgeRequest represents the fixed-size binary struct expected by sentinel-vmi
...
// HekiGuardResponse is the response from sentinel-vmi (JSON or binary.)
// Wait, if the request is binary, is the response binary? The user only specified the registration struct.
// Assume the response is a simple 1-byte success code.
// Read 1 byte for success.
...
func (h *HekiBridge) ProtectMapPages(maps *BPFMaps) error {
	...
	mapEntries := []struct {
		...
		{"vmi_alert_map", maps.VmiAlertMap, "/sys/fs/bpf/telos/vmi_alert_map", true},
	}
	...
	for _, entry := range mapEntries {
		...
		// or pass the value from Info if possible. 4096 is used.
	...
	h.connected = true
	log.Printf("[HEKI] [PASS] %d map pages under Ring -1 NPT write-protection", protectedCount)
	return nil
}

// sendBinaryRequest sends a binary request to sentinel-vmi and reads a 1-byte response.
func (h *HekiBridge) sendBinaryRequest(req HekiDrawbridgeRequest) error {
	...
	// Read response (8 bytes: Nonce. 0 = failure)
	resp := make([]byte, 8)
	if _, err := conn.Read(resp); err != nil {
		return fmt.Errorf("failed to read response: %w", err)
	}

	nonce := binary.LittleEndian.Uint64(resp)
	if nonce == 0 {
