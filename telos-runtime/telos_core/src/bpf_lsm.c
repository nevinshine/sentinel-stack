...
// Pinned alert map shared with sentinel-vmi: PID -> Threat Level
struct {
  __uint(type, BPF_MAP_TYPE_HASH);
  __uint(max_entries, 1024);
  __type(key, __u32);   // PID
...
// [Pillar 5] Policy-Carrying Code (PCC) AOT Registration map
struct inode_key {
  __u64 dev;
  __u64 ino;
};
...
int BPF_PROG(telos_check_exec, struct linux_binprm *bprm) {
  ...
  // === Pillar 5: PCC AOT Cryptographic Enforcement ===
  struct inode_key file_key = {};
  file_key.dev = BPF_CORE_READ(bprm, file, f_inode, i_sb, s_dev);
  file_key.ino = BPF_CORE_READ(bprm, file, f_inode, i_ino);

  __u32 *auth_status = bpf_map_lookup_elem(&pcc_auth_map, &file_key);
  if (!auth_status || *auth_status != 1) {
    emit_event(pid, TAINT_CRITICAL, 1, "pcc_unverified");
    return -EPERM;
  }

  // Check if this process has a Ring -1 VMI quarantine entry
  __u32 *vmi_threat = bpf_map_lookup_elem(&vmi_alert_map, &pid);
  if (!vmi_threat) {
    struct task_struct *current_task =
        (struct task_struct *)bpf_get_current_task();
    if (current_task) {
      __u32 ppid = BPF_CORE_READ(current_task, real_parent, tgid);
      vmi_threat = bpf_map_lookup_elem(&vmi_alert_map, &ppid);
    }
  }

  if (vmi_threat && *vmi_threat == VMI_THREAT_MALICIOUS) {
    emit_event(pid, TAINT_CRITICAL, 1, "vmi_lockdown_exec");
    return -EPERM;
  }
...
int BPF_PROG(telos_check_file, struct file *file) {
  ...
  // Check if this process has a Ring -1 VMI quarantine entry
  __u32 *vmi_threat = bpf_map_lookup_elem(&vmi_alert_map, &tracked_pid);
  if (!vmi_threat) {
    vmi_threat = bpf_map_lookup_elem(&vmi_alert_map, &pid);
  }

  if (vmi_threat && *vmi_threat == VMI_THREAT_MALICIOUS) {
    if (info && info->taint_level < TAINT_CRITICAL) {
      info->taint_level = TAINT_CRITICAL;
      bpf_map_update_elem(&process_map, &tracked_pid, info, BPF_EXIST);
    }
    emit_event(pid, TAINT_CRITICAL, 1, "vmi_lockdown_file");
    return -EPERM;
  }
...
int BPF_PROG(telos_check_connect, struct socket *sock, struct sockaddr *address,
  ...
  __u32 *vmi_threat = bpf_map_lookup_elem(&vmi_alert_map, &tracked_pid);
  if (!vmi_threat) {
    // Check actual current process PID
    vmi_threat = bpf_map_lookup_elem(&vmi_alert_map, &pid);
  }
  ...
  if (vmi_threat && *vmi_threat == VMI_THREAT_MALICIOUS) {
    ...
    emit_event(pid, TAINT_CRITICAL, 1, "vmi_lockdown");
    return -EPERM;
  }
...
int telos_sched_fork(struct trace_event_raw_sched_process_fork *ctx) {
  ...
  // Allocate and submit to shared sentinel_events ringbuf
  struct sentinel_event_t *sent_evt;
  sent_evt =
      bpf_ringbuf_reserve(&sentinel_events, sizeof(struct sentinel_event_t), 0);
  if (sent_evt) {
    sent_evt->event_type = SENTINEL_EVENT_PROCESS_VIOLATION;
    sent_evt->_padding = 0;
    sent_evt->timestamp_ns = bpf_ktime_get_ns();
    sent_evt->pid_tgid = bpf_get_current_pid_tgid();
    sent_evt->data.process_violation.parent_pid = parent_pid;
    sent_evt->data.process_violation.taint_signature = 0xCAFEBABE12345678;
...
int BPF_PROG(telos_check_ptrace, struct task_struct *child, unsigned int mode) {
