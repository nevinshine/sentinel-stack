...
def run():
    ...
    # 1. Start by manually configuring /etc/shadow in the kernel's Inode map 
    ...
    # directly assuming the Inode is protected for the demo, but actually one can
    # just rely on the fallback filename check 'id_' in bpf_lsm.c! Instead, use
    # a local mock file that is readable so the agent doesn't crash 
    # from standard Linux permissions before Telos can even intercept it. 

    # Create a fake SSH key to trigger the `id_` filename fallback heuristic
    ...
    try:
        ...
        # A 2-second timeout is used so it doesn't hang if dropped
