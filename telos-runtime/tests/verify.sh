...
if grep -q "Session mapped: ${SESSION_ID:0:8}" /tmp/cortex.log; then
    echo "[PASS] SUCCESS: Session mapped correctly."
else
    echo "[FAIL] FAILURE: Session mapping not found in Cortex logs."
    cat /tmp/cortex.log
    kill $CORTEX_PID $AGENT_PID
    exit 1
fi
...
if grep -q "Agent ${AGENT_REAL_PID} taint escalated to" /tmp/cortex.log; then
    echo "[PASS] SUCCESS: Agent taint escalated."
else
    echo "[FAIL] FAILURE: Agent taint escalation not found in Cortex logs."
    cat /tmp/cortex.log
    kill $CORTEX_PID $AGENT_PID
    exit 1
fi
