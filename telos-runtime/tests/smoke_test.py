...
def test_taint_pipeline():
    ...
    # Tainting the test PID would trigger the Network Slam on the next
    ...
    # This avoids tainting the test runner process.
    r_fd, w_fd = os.pipe()
    ...
    test("Network Slam blocks tainted process", exit_code == 42,
         "socket_connect returned -EPERM [PASS]" if exit_code == 42
...
def test_bulk_block_ips():
    ...
    # (metrics are sampled every 30s, so the IPC response is checked directly)
