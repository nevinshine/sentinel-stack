...
def main():
    ...
    try:
        ...
        if client.update_inode(passwd_ino, 4): # 4 = CRITICAL
            print(f"[PASS] Configured /etc/passwd (Inode {passwd_ino}) as SENSITIVE")
        else:
    ...
    try:
        ...
        if client.update_network(be_int, 1):
             print(f"[PASS] Configured 127.0.0.1 (Big Endian: {be_int}) as ALLOWED")

        # Little Endian (Native/Host Order on x86)
        le_int = struct.unpack("<I", packed_ip)[0]
        if client.update_network(le_int, 1):
             print(f"[PASS] Configured 127.0.0.1 (Little Endian: {le_int}) as ALLOWED")
    ...
    if client.send_update_taint(pid, 4): # 4 = CRITICAL
        print(f"[PASS] Tainted PID {pid} to Level 4")
    else:
    ...
    # So if ECONNREFUSED is returned, passed. If EPERM, failed.
