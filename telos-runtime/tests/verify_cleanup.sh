...
if ! command -v bpftool &> /dev/null; then
    echo "[FAIL] bpftool not found. Install linux-tools-common / linux-tools-generic"
    exit 1
fi
...
if [ -z "$MAP_ID" ]; then
    echo "[FAIL] Failed to find network_map. Is telos_daemon running?"
    kill $CORTEX_PID
    exit 1
fi
...
if [ "$POST_ADD_COUNT" -le "$INITIAL_COUNT" ]; then
    echo "[FAIL] Map count did not increase. Intent failed?"
    kill $CORTEX_PID
    exit 1
fi
...
if grep -q "Traceback" /tmp/cortex_cleanup.log; then
    echo "[FAIL] Cortex Error Detected"
