...
def cmd_start(extra_args: list[str]):
    ...
    if os.geteuid() != 0:
        console.print(f"  [bold {RED}][FAIL][/]  Please run as root (use sudo).")
        sys.exit(1)
    ...
    if not run_step("make bpf"):
        console.print(f"[{RED}][FAIL][/] Failed to build eBPF Core")
        sys.exit(1)
    ...
    if not run_step("make loader"):
        console.print(f"[{RED}][FAIL][/] Failed to build Go Loader")
        sys.exit(1)
    ms = int((time.time() - start_t) * 1000)
    console.print(f"[{DARK_GRAY}]│[/]")
...
def cmd_dash():
    ...
    if not tui_bin.exists():
        ...
        if not success:
            console.print(f"  [{RED}][FAIL][/]  Failed to build TUI")
            sys.exit(1)
...
def main():
    ...
    if command == "start":
        ...
        print_header()
        console.print(f"  [{RED}][FAIL][/]  Unknown command: [bold]{command}[/]")
        console.print()
        cmd_help()
        sys.exit(1)
