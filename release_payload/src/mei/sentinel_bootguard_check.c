...
bool sentinel_bootguard_is_verified_boot_active(void)
{
	...
	if (geteuid() != 0) {
		fprintf(stderr, "[BootGuard] Error: Root privileges required to read MSRs.\n");
		return false; /* Fail open? No, the caller should handle the error. But for safety, return true to block? Actually let's return true on error to be safe. */
	}

	fd = open("/dev/cpu/0/msr", O_RDONLY);
	if (fd < 0) {
		fprintf(stderr, "[BootGuard] Warning: Could not open /dev/cpu/0/msr.\n");
		fprintf(stderr, "[BootGuard] Make sure the 'msr' kernel module is loaded (modprobe msr).\n");
	...
	if (msr_val & B_BOOT_GUARD_VERIFIED_BOOT) {
		...
		printf("[BootGuard] The ME provides the cryptographic root of trust for this platform.\n");
		printf("[BootGuard] Disabling the ME will result in a hardware BRICK on next reboot.\n");
	...
	if (msr_val & B_BOOT_GUARD_MEASURED_BOOT) {
		...
		printf("[BootGuard] ME disable is potentially safe, though PCR registers will change.\n");
